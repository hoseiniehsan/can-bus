...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 11 21:50:50 2019
...
#from can import LimitedDurationCyclicSendTaskABC

"""
Warning: The functional API in can.broadcastmanager.send_periodic() is now 
deprecated and will be removed in version 4.0. Use the object oriented API
via can.BusABC.send_periodic() instead.
"""
...
def limited_periodic_send(bus):
    ...
    msg = can.Message(arbitration_id=0x12345678, data=[0, 0, 5, 5, 0, 0], is_extended_id=True)
...
if __name__ == '__main__':
    reset_msg = can.Message(arbitration_id=000, data=[0, 0, 0, 0, 0, 0],
                            is_extended_id=False)
    
    for interafce, channel in [
            ('socketcan', 'vcan0'),
            #('ixxat', 0)
            ]:
        print("Carring out cyclic tests with {} interface".format(interafce))
        
        bus = can.interface.Bus(interafce=interafce, channel=channel,
                                bitrate=500000)
        bus.send(reset_msg)
